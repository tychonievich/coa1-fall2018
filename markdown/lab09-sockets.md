...
As discussed in class `fcntl.h` and `unistd.h` provide functional wrappers around the internal operating-system abstraction of a file-like objects.
...
Among those are a set of related concepts called "sockets."
...
Addresses are somewhat involved to explain (we'll go into more on these in COA2),
...
- `ipOfServer.sin_addr.s_addr` needs to be `htonl(INADDR_ANY)` for the server to say it is listening for communication from any other computer; for the client it instead needs to be `inet_addr(ip_address_of_server);` where `ip_address_of_server` will be a string containing four numbers separated by periods, like `"128.143.67.241"`{.c}.
    - You can learn the IP address of a URL by using the `host` command line tool.
        ...
        ````bash
        $ host labsrv01.cs.virginia.edu
        labsrv01.cs.virginia.edu has address 128.143.67.241
        ````
...
Your job is to write a client program that

1. Accepts an IP address and a port number from the command line.
1. Connects to that server.
1. `read`s a message from the server and displays it on the command line.
...
If too many people leave too many servers running, the NX backend servers may eventually start running out of open ports and need to be restarted by the systems staff...
